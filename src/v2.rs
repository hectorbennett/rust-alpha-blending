...
pub fn blend_rgba(bg: Rgba, fg: Rgba) -> Rgba {
    ...
    let one_minus_alpha_fg = 1.0 - a_fg;

    // calculate final alpha
    let a = a_fg + a_bg * one_minus_alpha_fg;

    // calculate final colour channels
    let r = ((r_fg * a_fg) + (r_bg * a_bg * (one_minus_alpha_fg))) / a;
    let g = ((g_fg * a_fg) + (g_bg * a_bg * (one_minus_alpha_fg))) / a;
    let b = ((b_fg * a_fg) + (b_bg * a_bg * (one_minus_alpha_fg))) / a;
